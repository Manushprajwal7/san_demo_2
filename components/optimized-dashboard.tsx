...
// Static numbers – never change, defined once to avoid recalculation and reduce initial render
...
const COMPLIANCE_STATS = [
  { label: "Compliant", value: "87%", icon: CheckCircle, bg: "bg-green-50", valueColor: "text-green-600", labelColor: "text-green-700" },
  { label: "Warning", value: "8%", icon: AlertTriangle, bg: "bg-yellow-50", valueColor: "text-yellow-600", labelColor: "text-yellow-700" },
  { label: "Overdue", value: "5%", icon: Clock, bg: "bg-red-50", valueColor: "text-red-600", labelColor: "text-red-700" },
] as const;
...
export default function OptimizedDashboard() {
  ...
  // Read cached data for instant initial paint (placeholderData)
  const getCachedData = useCallback((): DashboardData | undefined => {
  ...
  // Optimized data fetching with React Query – cache for fast repeat visits
  const { data: dashboardData, isLoading, isFetching, error, refetch } = useQuery({
    ...
    staleTime: 2 * 60 * 1000, // 2 minutes – treat as fresh, reduce refetches
    gcTime: 10 * 60 * 1000, // 10 minutes cache
    retry: 2,
    refetchOnWindowFocus: false, // avoid refetch on tab focus for faster feel
  });

  // Memoized stat rows – deterministic trend by index for stable memo
  const systemStats = useMemo(() => {
    const tables = dashboardData?.system_tables;
    if (!tables) return [];
    return Object.entries(tables).map(([key, value], index) => ({
      key,
      value,
      icon: getTableIcon(key),
      color: getTableColor(index),
      trend: (index % 3 === 0 ? "up" : index % 3 === 1 ? "down" : "neutral") as "up" | "down" | "neutral",
      trendValue: `${(index % 5) * 5 + 5}%`,
    }));
  }, [dashboardData?.system_tables, getTableIcon, getTableColor]);
  ...
  }, [dashboardData?.employee_stats]);

  // Use static constants for charts – no dependency on dashboard data
  const performanceMetrics = PERFORMANCE_METRICS;
  const activityTimelineData = ACTIVITY_TIMELINE_DATA;

  // Show full skeleton only when loading and no data (including no cached placeholder)
    ...
    <div className="space-y-6">
          ...
          <ChartContainer title="Compliance Status" icon={Shield} description="Compliance metrics overview">
            <div className="grid grid-cols-3 gap-4 h-64">
              {COMPLIANCE_STATS.map((stat) => (
                <div key={stat.label} className={`flex flex-col items-center justify-center rounded-lg p-4 ${stat.bg}`}>
                  <stat.icon className={`h-8 w-8 mb-2 ${stat.valueColor}`} />
      ...
      {dashboardData.notice_tables.length > 0 && (
        <ChartContainer title="Data Tables Analysis" icon={Database} description={`${dashboardData.notice_tables.length} tables analyzed`}>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={dashboardData.notice_tables.map((t: TableStats) => ({
                name: t.display_name,
                rows: t.row_count,
            ...
            <ResponsiveContainer width="100%" height={300}>
              <Treemap
                data={dashboardData.notice_tables.map((t: TableStats) => ({
                  name: t.display_name,
                  size: t.row_count,
                }))}
                dataKey="size"
                ...
                fill="#8884d8"
              >
                {dashboardData.notice_tables.map((entry: TableStats, index: number) => (
